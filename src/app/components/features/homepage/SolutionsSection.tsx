...
export const SolutionsSection = () => {
    ...
    const solutionsData = t("home.solutions.list", { returnObjects: true }) as SolutionType[];
    ...
    const [isSlidesCenter, setIsSlidesCenter] = useState(true);

    const computeIsSlidesCenter = () => {
        setIsSlidesCenter(innerWidth > 767);
    };

    useEffect(() => {
        computeIsSlidesCenter();
        window.addEventListener("resize", computeIsSlidesCenter);
        return () => {
            window.removeEventListener("resize", computeIsSlidesCenter);
        };
    }, []);
    ...
    return (
        <section ref={sectionRef} id="solutions" className="pt-[200px] section xl:pt-[167px] md:pt-[110px] sm:pt-[90px]">
            <div className="container">
                ...
                <div
                    className={clsx(
                        "pl-[24.61%] mt-[-52px] mr-[-12px] flex gap-[39px] 2xl:pl-[26.11%] 2xl:mt-[-48px] md:pl-0 md:mt-0 md:justify-between sm:block sm:m-0",
                        { "[&]:pl-[18%] md:[&]:pl-0": isRussianLocale }
                    )}
                >
                    <div
                        className={clsx(
                            "mb-[200px] relative w-full max-w-[750px] flex-none h-[699px] 2xl:max-w-[673px] 2xl:h-[587px] xl:mb-[167px] lg:max-w-[543px] md:mb-[275px] md:h-[516px] md:max-w-[450px] tb:mb-[165px] sm:ml-[-20px] sm:mr-[-20px] sm:w-[auto] sm:h-fit sm:mb-[30px] sm:max-w-[unset] xs:w-[calc(100%+244px)]",
                            {
                                "[&]:max-w-[840px] lg:[&]:max-w-[543px] lg:[&]:h-[448px] tb:[&]:max-w-[460px] tb:[&]:h-[360px] sm:[&]:max-w-[unset] sm:[&]:h-fit":
                                    isRussianLocale,
                            }
                        )}
                    >
                        <div
                            className={clsx(
                                ...
                                { "opacity-0": !isInView },
                                { "delay-500": isInView },
                                { "lg:[&]:h-[220px] md:[&]:h-[173px] tb:[&]:translate-y-[-10%] tb:[&]:h-[120px]": isRussianLocale }
                            )}
                        ></div>
                        <div
                            className={clsx(
                                ...
                                { "delay-500": isInView },
                                {
                                    "[&]:translate-y-[38%] 2xl:[&]:translate-y-[45%] lg:[&]:h-[200px] lg:[&]:translate-y-[35%] md:[&]:translate-y-[-10%] tb:[&]:h-[120px]":
                                        isRussianLocale,
                                }
                            )}
                        ></div>
                        <Swiper
                            ...
                            simulateTouch={false}
                            loop
                            centeredSlides={isSlidesCenter}
                            centeredSlidesBounds={false}
                            className="w-full h-full solutions-swiper"
                                        ...
                                        >
                                            <span
                                                className={clsx(
                                                    ...
                                                    {
                                                        "[&]:text-[36px] xl:[&]:text-[32px] lg:[&]:text-[24px] tb:[&]:text-[20px] sm:[&]:text-[18px] xxs:[&]:text-[16px]":
                                                            isRussianLocale,
                                                    }
                                                )}
                                                dangerouslySetInnerHTML={{ __html: solutionsData[cleanIndex].name }}
                                            ></span>
                                            <motion.span
                                                ...
                                                className={clsx(
                                                    ...
                                                    { "[&]:text-[80px] 2xl:[&]:text-[64px] lg:[&]:text-[48px] tb:[&]:text-[40px]": isRussianLocale }
                                                )}
                                                dangerouslySetInnerHTML={{ __html: solutionsData[cleanIndex].name }}
                                            ></motion.span>
                                        </button>
                                ...
                                return (
                                    index === activeIndex && (
                                        ...
                                        >
                                            <h3
                                                className={clsx(
                                                    "mb-[20px] h2 hidden sm:block  sm:mr-[-5px] sm:text-[60px] sm:leading-[64px]",
                                                    {
                                                        "sm:text-[64px]": isGaming,
                                                    },
                                                    { "sm:[&]:text-[48px] sm:[&]:leading-[130%] xxs:[&]:text-[40px]": isRussianLocale }
                                                )}
                                                dangerouslySetInnerHTML={{ __html: solutionsData[cleanIndex].name }}
                                            ></h3>
                                            <p
                                                className={clsx(
                                                    ...
                                                    { "sm:[&]:max-w-[320px]": isRussianLocale }
                                                )}
                                                dangerouslySetInnerHTML={{ __html: solutionsData[cleanIndex].text }}
                                            />
                                            <ResponsiveImage
                                                className={clsx(
                                                    ...
                                                    {
                                                        "mb-[-3px] translate-x-[-1.9%] sm:w-[283px] sm:h-[283px] sm:translate-x-[37px]": isSubscription,
                                                    }
                                                )}
                                                src={solution.image}
