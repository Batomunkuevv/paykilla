...
const ArrowIcon = () => (
    <span className="flex items-center w-[31px] flex-none tb:w-[25px] sm:px-[2px] sm:w-[24px]">
        <svg className="sm:mt-[1px]" viewBox="0 0 31 26" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export const Button = (props: ButtonProps) => {
    const { className, children, isDisabled, isLoading, type = "button", ...rest } = props;
    const classes = clsx(
        "py-[21px] px-[40px] flex items-center justify-center gap-x-[10px] w-fit min-h-[100px] min-w-[390px] bg-orange border-2 border-transparent text-button text-t-white transitionl-all duration-300 outline-none",
        "hover:bg-orange-hover focus:bg-orange-focus focus:border-orange-hover disabled:bg-disabled",
        { "bg-orange-focus border-orange-hover pointer-events-none": isLoading },
    ...
    if ("href" in props && typeof props.href === "string") {
        return (
            <Link href={props.href} target={props.target} className={classes}>
                {children}
                {isLoading ? <Spinner className="w-[26px] h-[26px] flex-none tb:w-[20px] tb:h-[20px]" /> : <ArrowIcon />}
            </Link>
        );
    }
